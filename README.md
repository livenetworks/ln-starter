...
> The auth pages render **before** any of this. The shipped layout asks your
> application's own Vite service for the tags — so a custom hot file, build
> directory or manifest name is honoured — and emits nothing when the entries
> have not been built. A fresh install, or a deploy where the asset build was
> skipped, gets an unstyled but working login page rather than HTTP 500 on the
