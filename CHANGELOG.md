...
- `DocumentationContractTest`: compares structured data extracted from the real enums, config and command registrations against the documents. It fails on an event name, reason code, environment variable, audit column or artisan command that the docs name but the code does not define, on a documented envelope field absent from the real envelope, on a forbidden field appearing in a query example, and on a broken relative link. It also enforces the executable fresh-adoption order (publish config, enable auth, install, migrate, run readiness, verify a package-specific route) and compares the runbook's structured `reject()` event/reason mapping with the state machine
