...
- `DocumentationContractTest`: compares structured data extracted from the real enums, config and command registrations against the documents. It fails on an event name, reason code, environment variable, audit column or artisan command that the docs name but the code does not define, on a documented envelope field absent from the real envelope, on a forbidden field appearing in a query example, and on a broken relative link. It also asserts a procedural precondition (the adoption documents must tell the reader to enable the auth module, because the default is `false` and both the installer and readiness silently skip every auth check when it is off) and one event-to-reason pairing extracted from the state machine (reason codes routed through `reject()` are staged as `auth.magic.proof.rejected`, so a runbook may not file them under `proof.replayed`)
...
### Fixed
- Scan only the **status block** of the active changelog section at finalisation — the lines between the heading and the first `###` — instead of the whole section. Entries are prose about what changed and may legitimately contain the words a status block may not; this release's own entries describe handling "release candidate" wording and "Unreleased" sections, so the previous scan made 2.0.0 undatable by its own description of itself
- Normalise CRLF before matching dated headings. `$` in `/m` mode matches before a line feed but not before a carriage return, so on a Windows checkout every heading pattern was unmatchable and finalisation was permanently impossible. Fixture documents are written with LF, which is why only running the gate against the real files exposed it
